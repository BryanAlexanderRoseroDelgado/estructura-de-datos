...
vector<string> ListaCircularDoble::generarArreglo(const string& clave) {
    vector<string> arreglo;
    if (!cabeza) return arreglo;

    Nodo* actual = cabeza;
    do {
        if (clave == "titulo") {
            arreglo.push_back(actual->libro.getTitulo());
        } else if (clave == "isbn") {
            arreglo.push_back(actual->libro.getIsbn());
        } else if (clave == "isni") {
            arreglo.push_back(actual->libro.getAutor().getIsni());
        } else if (clave == "nombreAutor") {
            arreglo.push_back(actual->libro.getAutor().getNombre());
        } else if (clave == "fechaNac") {
            arreglo.push_back(actual->libro.getAutor().getFechaNacimiento().mostrar());
        }else if (clave == "fechaPub") {
            arreglo.push_back(actual->libro.getFechaPublicacion().mostrar());
        } 
        actual = actual->siguiente;
    } while (actual != cabeza);
